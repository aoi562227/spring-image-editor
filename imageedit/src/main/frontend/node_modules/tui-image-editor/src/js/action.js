    ...
    return extend(
      {
        ...
        retrieve: () => {
          window.alert("프로젝트 가져오기");
        },
        upload: async (email, file) => {
          ...
          let stackDataStructure = this.getUndoStack().concat(this.getRedoStack());
          let commandJson = JSON.stringify(stackDataStructure);
          
          let canvas = this._graphics.getCanvas();
          let canvasJson = canvas.toJSON();
          canvasJson = JSON.stringify(canvasJson);

          let projectJson = `{"stackData":` + commandJson + `,"canvasData":` + canvasJson + "}";
          console.log("JSON 결과 : " + projectJson);
          
          // JSON 추가
          formData.append('projectData', new Blob([projectJson], {
          ...
          await axios
          // /save 주소로 데이터 전달
            .post("/save", formData, config)
            .then((response) => {
              // 성공했다면
              return response.data; // 6자리 코드?
              /*
              if (response.data) {
